...
test('classExports', (t) => {
  return exec('jsdoc -c ./config.json', {
    ...
    return fs.readFile('test/testfiles/class/README.md', 'utf8').then((content) => {
      return fs.readFile('test/testfiles/class/expected.md', 'utf8').then((expected) => t.is(content, expected))
    })
  })
})
...
test('functionExports', (t) => {
  return exec('jsdoc -c ./config.json', {
    ...
    return fs.readFile('test/testfiles/function/README.md', 'utf8').then((content) => {
      return fs.readFile('test/testfiles/function/expected.md', 'utf8').then((expected) => t.is(content, expected))
    })
  })
})
